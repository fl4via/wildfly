...
public class InfinispanBeanManager2<I, T> implements BeanManager<I, T, TransactionBatch> {
    ...
    private static ThreadFactory createThreadFactory() {
        PrivilegedAction<ThreadFactory> action = () -> new JBossThreadFactory(new ThreadGroup(
                InfinispanBeanManager2.class.getSimpleName()), Boolean.FALSE, null, "%G - %t", null, null);
    ...
    private volatile Scheduler2<I> scheduler;
    private volatile ExecutorService executor;
    private volatile CommandDispatcher<Scheduler2<I>> dispatcher;

    public InfinispanBeanManager2(InfinispanBeanManagerConfiguration<I, T> configuration, IdentifierFactory<I> identifierFactory, Configuration<BeanKey<I>, BeanEntry<I>, BeanFactory<I, T>> beanConfiguration, Configuration<BeanGroupKey<I>, BeanGroupEntry<I, T>, BeanGroupFactory<I, T>> groupConfiguration) {
    ...
    public void start() {
        ...
        List<Scheduler2<I>> schedulers = new ArrayList<>(2);
        Duration timeout = this.expiration.getTimeout();
        if ((timeout != null) && !timeout.isNegative()) {
            schedulers.add(new BeanExpirationScheduler2<>(this.batcher, new ExpiredBeanRemover<>(this.beanFactory), this.expiration));
        }
        ...
        if (idleTimeout != null) {
            ...
            if (!idleDuration.isNegative()) {
                schedulers.add(new EagerEvictionScheduler2<>(this.beanFactory, this.groupFactory, this.expiration.getExecutor(), idleDuration, this.dispatcherFactory, dispatcherName + "/eager-passivation"));
            }
        }

        this.scheduler = new CompositeScheduler2<>(schedulers);
    ...
    private void schedule(Bean<I, T> bean) {
        if (this.dispatcher != null) {
            ...
            try {
                this.executeOnPrimaryOwner(bean, new ScheduleSchedulerCommand2<>(bean));
            } catch (Exception e) {
    ...
    void remove(Bean<I, T> bean) {
        if (this.dispatcher != null) {
            final Node node = locatePrimaryOwner(bean.getId());
            if (dispatcher.isLocal(node)) {
                scheduler.remove(bean.getId());
                return;
            }
            try {
                this.executeOnPrimaryOwner(bean, new RemoveSchedulerCommand2<>(bean));
            } catch (Exception e) {
    ...
    private void executeOnPrimaryOwner(Bean<I, T> bean, final Command<Void, Scheduler2<I>> command) throws CommandDispatcherException {
        CommandDispatcher<Scheduler2<I>> dispatcher = this.dispatcher;
        ExceptionSupplier<CompletionStage<Void>, CommandDispatcherException> action = new ExceptionSupplier<CompletionStage<Void>, CommandDispatcherException>() {
            ...
            public CompletionStage<Void> get() throws CommandDispatcherException {
                ...
                Node node = InfinispanBeanManager2.this.locatePrimaryOwner(bean.getId());
    ...
    public void dataRehashed(DataRehashedEvent<BeanKey<I>, BeanEntry<I>> event) {
        ...
        if (event.isPre()) {
            ...
            Runnable cancelTask = new Runnable() {
                @Override
                public void run() {
                    InfinispanBeanManager2.this.cancel(newLocality);
                }
            };
            try {
            ...
            Runnable scheduleTask = new Runnable() {
                @Override
                public void run() {
                    InfinispanBeanManager2.this.schedule(oldLocality, newLocality);
                }
            };
            try {
    ...
    private class SchedulableBean implements Bean<I, T> {
        ...
        public void close() {
            ...
            if (!this.bean.isValid()) {
                InfinispanBeanManager2.this.remove(this.bean);
            } // if bean is valid, expiration and eviction are already scheduled
            else {
                InfinispanBeanManager2.this.schedule(this.bean);
            }
        }
    }
}
